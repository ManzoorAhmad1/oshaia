...
export default function AdminSettingsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isAdmin) {
        // Admin uses /admin/credentials (email + password, no current password needed)
        ...
        // Staff uses /auth/change-password (requires current password)
        if (!currentPassword) { setError('Current password is required'); return; }
        if (!password) { setError('New password is required'); return; }
        await api.patch('/auth/change-password', { currentPassword, newPassword: password });
      }
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-xl">
      <div>
        <h1 className="text-2xl font-bold text-[#112b38]">Settings</h1>
        <p className="text-gray-500 text-sm mt-0.5">
          {isAdmin ? 'Update your admin email and password.' : 'Change your account password.'}
        </p>
      </div>
      ...
      <form onSubmit={handleSave} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        {/* Staff only: current password required */}
        {!isAdmin && (
          <div>
